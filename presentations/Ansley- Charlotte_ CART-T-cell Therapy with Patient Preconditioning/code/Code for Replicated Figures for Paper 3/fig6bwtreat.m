% replicate figure6b treatment

% 5 days of chemo + 3 days of rest + cart injection on day 9

% chemo: high strength for one half hour each day for 5 days (C5), 75
...
index4 = find(tspan==c4);index5 = find(tspan==c5);index6 = find(tspan==c6);

%cart injecttion day(s)

c7 = 9;

index7 = find(tspan==c7);
...
% we are done with chemo, now 4 days rest, just one ode

t6 = tspan(index5+1:index6);%days from 6-8
...
t7 = tspan(index6+1:index7);%days from 6-8
...
[~,y] = ode23s(@(t,y) GetHDLM(t,y),t8,[T0,E0,CT0,M0]);
